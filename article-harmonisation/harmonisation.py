...
NODE_ROLES = ['Researcher', "Comparer", "Meta description", "Title", "Content guidelines", "Writing guidelines"]
...
ROOT = os.getcwd()
EXTRACTED_TEXT_DIRECTORY = f"{ROOT}/content-optimization/data/02_intermediate/all_extracted_text/"
...
class GraphState(TypedDict):
    articles: Optional[list] #A list where each element is a String with the article content
    keypoints: Optional[list] #A list where each element is a String with the article keypoints
...
def decide_next_node(state):
    return "researcher_node" if state.get('counter') < article_list_length else "compiler_node"

def decide_next_optimisation_node(state):
    if state.get('content_guidelines_optimisation') == True:
        return "content_guidelines_node" 
    elif state.get('title_optimisation') == True:
        return "title_optimisation_node"
    elif state.get("meta_desc_optimisation") == True:
