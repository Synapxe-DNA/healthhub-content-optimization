...
def merge_ground_truth_to_data(
        ...
        weighted_embeddings: pd.DataFrame) -> pd.DataFrame:
    """
...
def generate_clusters(merged_df_with_groundtruth: pd.DataFrame, 
                      ...
                      weight_kws: float,
                      set_threshold: Union[None, float])-> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.Series]:
    """
...
def generate_subclusters(weighted_embeddings: pd.DataFrame,
                         pred_cluster: pd.DataFrame, 
                         umap_parameters: Dict)-> pd.DataFrame:
    """
    ...
    final_result_df = process_all_clusters(cluster_morethan10_embeddings,umap_parameters)
    final_result_df_with_numbers = assign_unique_numbers_to_topics(final_result_df, pred_cluster)
    ...
    updated_pred_cluster = pd.merge(pred_cluster, new_cluster_to_merge, how='left', on='id')
    ...
    df_cluster_kws_na = updated_pred_cluster[updated_pred_cluster['new_cluster'].isin(cluster_mt_1) & updated_pred_cluster['cluster_kws'].isna()]
...
def update_edges_dataframe(clustered_nodes: pd.DataFrame, 
                           updated_pred_cluster: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    unclustered_df = updated_pred_cluster[updated_pred_cluster['new_cluster'].isin(single_article_cluster)]
    ...
    clustered_df_new = pd.merge(clustered_nodes, updated_pred_cluster[['id','new_cluster','cluster_kws']], 
                                left_on='node_1_id',
                                right_on='id',
                                how='left').merge(
    ...
    final_clustered_nodes = clustered_df_new.drop(columns=['node_1_pred_cluster', 'node_2_pred_cluster'])    
    final_clustered_nodes = final_clustered_nodes.rename(columns={'new_cluster_1':'node_1_pred_cluster_new','new_cluster_2':'node_2_pred_cluster_new',
                                                                  'cluster_kws_1':'node_1_cluster_kws','cluster_kws_2':'node_2_cluster_kws'})
    final_clustered_nodes = final_clustered_nodes[final_clustered_nodes['node_1_pred_cluster_new'].isin(group_article_cluster) | final_clustered_nodes['node_2_pred_cluster_new'].isin(group_article_cluster)]
