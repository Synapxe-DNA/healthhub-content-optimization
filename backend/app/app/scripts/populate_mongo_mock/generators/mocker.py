...
from app.models.cluster import ClusterPopulated, Cluster
from app.models.edge import Edge
from app.models.harmonise import Harmonise
from app.models.optimise import Optimise
from app.scripts.populate_mongo_mock.generators.string_generator import random_str, random_id
...
class Mocker:
    ...
    def __init__(
        ...
        assert min_articles_per_cluster>0, "Number of articles must be greater than 0!"
        assert max_articles_per_cluster>=min_articles_per_cluster, "Max must be MEQ min articles!"
    ...
    def __create_article_ids(self) -> List[str]:
        num_articles = random.randint(0, (self.max_articles_per_cluster-self.min_articles_per_cluster))
        ...
        for i in range(self.num_clusters):
            ...
            articles = [
                Article(
                    ...
                    labels=random.sample(labels, random.randint(0, len(labels))),
                    cover_image_url='url',
            ...
            edges = [
                ...
                [random.sample(article_ids, 2) for _ in range(int(self.percent_connection*(len(article_ids)**2)))]
            ]
            ...
            harmonise_jobs = [
                Harmonise(
                    name=random_str(16),
                    article_ids=random.sample(article_ids, random.randint(int(len(article_ids)/2), len(article_ids)))
                )
            ]
            ...
            optimise_jobs = [
                Optimise(
                    article_id=x
                ) for x in random.sample(article_ids, random.randint(0, int(len(article_ids)/2)))
            ]
