...
from .beanie_documents import ArticleDocument, EdgeDocument, ClusterDocument
import warnings

class DBPopulator:
    def __init__(self, mongo_connector, articles_file_path, edges_file_path, cluster_file_path):
        ...
        self.client = AsyncIOMotorClient(mongo_connector._MongoConnector__connection_string)
    ...
    async def populate_articles(self):
        ...
        articles_df = articles_df.fillna({
            column: "" for column in articles_df.select_dtypes(include=["object"]).columns
        }).fillna({
            column: -1 for column in articles_df.select_dtypes(include=["float64", "int64"]).columns
        })

        articles_df = articles_df[articles_df['pr_name'].str.contains('Health Promotion Board')]
        articles_df['keywords'] = articles_df['keywords'].apply(lambda x: x.split(',') if x else [])
        articles_df['keywords'] = articles_df['keywords'].apply(lambda x: [e.strip() for e in x if e!=''])
    ...
    async def populate_edges(self):
        ...
        for _,row in edges_df.iterrows():
            ...
            articles_pair = await ArticleDocument.find(In(ArticleDocument.title, [node_1,node_2])).to_list()
            
            # Assume page title is unique
            if len(articles_pair) !=2:
                raise ValueError(f"Expected exactly 1 article for each of {node_1} and {node_2}, "
    ...
    async def populate_clusters(self):
        ...
        for row in cluster_pkl:
            articles_incluster = await ArticleDocument.find(In(ArticleDocument.title, row['titles'])).to_list()
            match_start = await EdgeDocument.find(In(EdgeDocument.start.id, [a.id for a in articles_incluster])).to_list()
            match_start_id_list = [e.id for e in match_start]
            match_end = await EdgeDocument.find(In(EdgeDocument.end.id, [a.id for a in articles_incluster])).to_list()
